...
{
	Frustum f;
	render->Push3DView( setup, 0, false, NULL, f );
	materials->GetMatrix( MATERIAL_VIEW, &view );
	materials->GetMatrix( MATERIAL_PROJECTION, &proj );
...
{
	l.m_Type=MATERIAL_LIGHT_DIRECTIONAL;
	l.m_Color=color;
	l.m_Direction=dir;
	l.m_Range=0.0;									// infinite
	l.m_Attenuation0=1.0;
	l.m_Attenuation1=0;
	l.m_Attenuation2=0;
	l.RecalculateDerivedValues();
}
#endif
...
//-----------------------------------------------------------------------------
void CUtlBufferEditor::SetExternalBuffer( void* pMemory, int nSize, int nInitialPut, int nFlags )
{
...
//-----------------------------------------------------------------------------
void CUtlBufferEditor::AssumeMemory( void *pMemory, int nSize, int nInitialPut, int nFlags )
{
#ifdef SHADER_EDITOR_DLL_2006
	Assert(0);
#else
...
//-----------------------------------------------------------------------------
void CUtlBufferEditor::EnsureCapacity( int num )
{
...
//-----------------------------------------------------------------------------
void CUtlBufferEditor::Get( void* pMem, int size )
{
	if ( CheckGet( size ) )
	{
...
//-----------------------------------------------------------------------------
int	CUtlBufferEditor::PeekLineLength()
{
	if ( !IsValid() )
		return 0;
...
//-----------------------------------------------------------------------------
bool CUtlBufferEditor::CheckGet( int nSize )
{
...
//-----------------------------------------------------------------------------
bool CUtlBufferEditor::CheckPeekGet( int nOffset, int nSize )
{
	if ( m_Error & GET_OVERFLOW )
		return false;
...
int CUtlBufferEditor::Scanf( const char* pFmt, ... )
{
	va_list args;
...
// It also places " at the beginning and end of the string
//-----------------------------------------------------------------------------
inline void CUtlBufferEditor::PutDelimitedCharInternal( CUtlCharConversion *pConv, char c )
{
...
void CUtlBufferEditor::Printf( const char* pFmt, ... )
{
	va_list args;
...
//-----------------------------------------------------------------------------
void CUtlBufferEditor::SetOverflowFuncs( UtlBufferOverflowFunc_t getFunc, UtlBufferOverflowFunc_t putFunc )
{
...
// Calls the overflow functions
//-----------------------------------------------------------------------------
bool CUtlBufferEditor::OnPutOverflow( int nSize )
{
	return (this->*m_PutOverflowFunc)( nSize );
}

bool CUtlBufferEditor::OnGetOverflow( int nSize )
{
...
//-----------------------------------------------------------------------------
bool CUtlBufferEditor::PutOverflow( int nSize )
{
	if ( m_Memory.IsExternallyAllocated() )
	{
		if ( !IsGrowable() )
...
bool CUtlBufferEditor::GetOverflow( int nSize )
{
	return false;
}
...
// Checks if a put is ok
//-----------------------------------------------------------------------------
bool CUtlBufferEditor::CheckPut( int nSize )
{
...
void CUtlBufferEditor::SetBigEndian( bool bigEndian )
{
	m_Byteswap.SetTargetBigEndian( bigEndian );
}

bool CUtlBufferEditor::IsBigEndian( void )
{
